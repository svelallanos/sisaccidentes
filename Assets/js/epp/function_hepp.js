...
function save() {
  formS = $("#form_save").submit(function (e) {
    ...
    const request = axios.post(base_url + "Epp/saveHEpp", formData);
...
function eliminar() {
  $(document).on("click", ".__delete", function () {
    ...
    Swal.fire({
      ...
      text: "¿Estas seguro de eliminar este EPP asginado, no podrás revertir esto?",
      icon: "info",
      ...
      if (result.isConfirmed) {
        ...
        const request = axios.post(base_url + "Epp/delHEpp", formData);
        request.then((res) => {
          if (res.data.status) {
